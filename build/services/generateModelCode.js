 ...
 */
async function generateModelCode(params) {
    try {
 ...
 */
function generateTypeScriptInterface(modelName, schema, swaggerDefinition, indentLevel = 0) {
    ...
    ['allOf', 'anyOf', 'oneOf'].forEach(key => {
        if (Array.isArray(schema[key])) {
            ...
            if (key === 'allOf') {
                schema[key].forEach((subSchema) => {
                    if (subSchema.$ref) {
                        ...
                        result = result.replace(`export interface ${formatInterfaceName(modelName)} {`, `export interface ${formatInterfaceName(modelName)} extends ${formatInterfaceName(refModelName)} {`);
                    }
            ...
            // For anyOf and oneOf, we use union types
                ...
                const unionTypes = schema[key].map((subSchema) => {
                    if (subSchema.$ref) {
                        const refModelName = getRefModelName(subSchema.$ref);
 ...
 */
function generateNestedTypes(schema, swaggerDefinition, indentLevel = 0) {
 ...
 */
function getTypeScriptType(schema, swaggerDefinition) {
    if (schema.$ref) {
