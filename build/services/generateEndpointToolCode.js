 ...
 */
async function generateEndpointToolCode(params) {
    try {
        const { path: endpointPath, method, swaggerFilePath, includeApiInName = false, includeVersionInName = false, singularizeResourceNames = true } = params;
 ...
 */
function findEndpoint(swaggerDefinition, endpointPath, method) {
 ...
 */
function generateToolName(method, path, operationId, includeApiInName = false, includeVersionInName = false, singularizeResourceNames = true) {
    const methodPrefix = {
        'GET': 'get',
        'POST': 'create',
        'PUT': 'update',
 ...
 */
function generateInputSchema(swaggerDefinition, endpoint) {
    const inputSchema = {
        type: 'object',
        properties: {},
        required: []
 ...
 */
function extractModelSchema(swaggerDefinition, modelName) {
 ...
 */
function processSchema(swaggerDefinition, schema) {
    if (!schema) {
        return { type: 'object' };
    }
    // Handle $ref
 ...
 */
function isLikelyUnmarshaler(swaggerDefinition, modelName) {
    const model = swaggerDefinition.definitions?.[modelName] || swaggerDefinition.components?.schemas?.[modelName];
 ...
 * @returns The processed schema
 */
function processUnmarshalerType(swaggerDefinition, modelName) {
    const model = swaggerDefinition.definitions?.[modelName] || swaggerDefinition.components?.schemas?.[modelName];
 ...
 */
function mapSwaggerTypeToJsonSchema(swaggerType) {
    const typeMap = {
        'integer': 'integer',
        'number': 'number',
        'string': 'string',
 ...
 */
function generateToolDefinition(toolName, endpoint, inputSchema) {
 ...
 */
function generateHandlerFunction(toolName, method, endpointPath) {
    return `
// Tool handler
